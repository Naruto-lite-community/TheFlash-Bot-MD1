...
//main.js
methodCode1: 'MÉTODO DEDESEA CONECTARSE?',
methodCode3: 'Opción',
...
//_autodetec.js
smsAutodetec1: (usuario, m) => `*» ${usuario}*\n*لقد قام ذلك المستخدم بتغير اسم المجموعه*\n\n🔰 *التي كانت اسمها:*\n*${m.messageStubParameters[0]}*`, 
smsAutodetec2: (usuario, groupMetadata) => `*» ${usuario}*\n*تم تغير صوره هذه المجموعه:*\n*${groupMetadata.subject}*`, 
smsAutodetec3: (usuario, m) => `*» ${usuario}*\n*تم تغير وصف المجموعه بنجاح*\n🔰 *الوصف الجديد الان:*`, 
smsAutodetec4: (usuario, m, groupMetadata) => `🔒 ${usuario}*\n*المجموعه ${m.messageStubParameters[0] == 'on' ? 'المشرفين فقط' : 'عام للجميع'} المجموعه ${groupMetadata.subject}*`, 
smsAutodetec5: (groupMetadata, usuario) => `*المجموعه الان ${groupMetadata.subject} تم تغيرها بواسطه:*\n*» ${usuario}*`, 
smsAutodetec6: (m) => `الجروب حالته*${m.messageStubParameters[0] == 'on' ? 'مقفل الان 🔒' : 'مفتوح الان 🔓'}*\n ${m.messageStubParameters[0] == 'on' ? 'الادمنز و المشرفين فقط يمكنهم التحدث' : 'الكل يستطيع التحدث الان'} في هذه المجموعه*`, 
smsAutodetec7: (m, usuario) =>  `@${m.messageStubParameters[0].split`@`[0]} لقد اصبحت الآن مشرفا\n\nالي رفعك ادمن ${usuario}`, 
smsAutodetec8: (m,  usuario) => `@${m.messageStubParameters[0].split`@`[0]} لم تعد مشرفا\n\nالي نزلك من الادمن ${usuario}`, 
...
//_antiviewonce.js
antiviewonce: (type, fileSize, m, msg) => `🕵️‍♀️ *فاكرني مش شايفك* 🕵️\n
🚫 *لا شي يخفي هنا* ${type === 'imageMessage' ? '`Imagen` 📷' : type === 'videoMessage' ? '`Vídeo` 🎥' : type === 'audioMessage' ? '`Mensaje de voz` 🔊' : 'este mensaje'}
- *الحجم:* \`\`\`${fileSize}\`\`\`
